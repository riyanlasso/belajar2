...
class MainWindow(QMainWindow):
    ...
    def hitungKNN(self) :
        ...
        sqlname = f"SELECT DISTINCT trainingknnvariables.name from trainingknnvariables.trainingknnvariables"
        ...
        # sqltesting = f"SELECT DISTINCT testingknnvariables.name from testingknnvariables.testingknnvariables"
        # cursor2.execute(sqltesting)
        # resultnametesting = cursor2.fetchall()
        # name_list_fix_testing = [row2[0] for row2 in resultnametesting]
        sqltesting = "SELECT DISTINCT name FROM testingknnvariables.testingknnvariables"
        cursor2.execute(sqltesting)
        resultnametesting = cursor2.fetchall()
        name_list_fix_testing = [row2[0] for row2 in resultnametesting]
        ...
        for cekTabel in range(len(name_list_fix)):
            ...
            for i in range(len(pilihMarker)):
                ...
                sql1 = f"SELECT trainingknnvariables.Name, trainingknnvariables.Average_Age, trainingknnvariables.Average_Height, trainingknnvariables.Average_Weight, trainingknnvariables.Marker,trainingknnvariables.AverageMarker FROM trainingknnvariables.trainingknnvariables WHERE trainingknnvariables.Marker = '{pilihMarker[i]}' AND trainingknnvariables.Name = '{name_list_fix[cekTabel]}'"
                cursor.execute(sql1)
                result1 = cursor.fetchall()

                # Query untuk mendapatkan data dari tabel testingknnvariables
                # sql2 = f"SELECT testingknnvariables.Name, testingknnvariables.Average_Age, testingknnvariables.Average_Height, testingknnvariables.Average_Weight, testingknnvariables.Marker, testingknnvariables.AverageMarker FROM testingknnvariables.testingknnvariables WHERE testingknnvariables.Marker = '{pilihMarker[i]}' AND testingknnvariables.Name = '{name_list_fix_testing}'"
                # cursor2.execute(sql2)
        ...
        #     label = data_labels[neighbor]
        #     if label in votes:
        #         votes[label] += 1
        #     else:
        #         votes[label] = 1

        # # Menentukan kelas dengan suara terbanyak
        # majority_vote = max(votes, key=votes.get)

        # # Output hasil klasifikasi
        # print(f"Hasil klasifikasi untuk orang saat ini: {majority_vote}")
