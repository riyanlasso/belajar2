        ...
        if i in frame_no_list:
            ...
            for j, marker in enumerate(markers):
                ...
                if marker in markers_fix:
                    ...
                    for y in range(len(y_data)-1):
                        if y_data[y] < y_data[y+1] :
                            skorY = y_data[y] - y_data[y+1]
                            scoringYdata.append(skorY)
                        elif y_data[y] > y_data[y+1] :
                            skorY = y_data[y] - y_data[y+1]
                            scoringYdata.append(skorY)
                        # print(f"Skor untuk koordinat X marker {namaMarker} adalah {skor}")
                        else : 
                            continue
                    
                    
                    for z in range(len(z_data)-1):
                        if z_data[z] < z_data[z+1] :
                            skorZ = z_data[z] - z_data[z+1]
                            scoringZdata.append(skorZ)
                        elif z_data[z] > z_data[z+1] :
                            skorZ = z_data[z] - z_data[z+1]
                            scoringZdata.append(skorZ)
                        # print(f"Skor untuk koordinat X marker {namaMarker} adalah {skor}")
                        else : 
                            continue    
                    print(f"Skoring X untuk {namaMarker} adalah {skorX}")
                    print(f"Skoring Y untuk {namaMarker} adalah {skorY}")
                    print(f"Skoring Z untuk {namaMarker} adalah {skorZ}")
