...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.input_button_training = QPushButton("Input Data Training")
        self.input_button_training.clicked.connect(self.show_data_training)
        ...
        self.plot_button = QPushButton("Show Testing Data")
    ...
    def show_data_training(self):
                ...
                for i in range(len(pilihMarker)):
                    ...
                    for row in result:
                        with connection2.cursor() as cursor2:
                            ...
                            # connection2.commit()        
        # berikan warna yang berbeda                 
        self.plot_widget_training_x.plot(all_marker_x,frame_no_list,pen=None, symbol='o', symbolSize=15,symbolBrush='r')
        # self.plot_widget_training_y.plot(all_marker_y,frame_no_list,pen=None, symbol='s', symbolSize=20,symbolBrush='y')
        # self.plot_widget_training_z.plot(all_marker_z,frame_no_list,pen=None, symbol='t', symbolSize=20,symbolBrush='b')
        ...
        # self.plot_widget.plot(frame_no_list, LASIX,
        #                       pen='g', symbol='o', symbolSize=10)
    def calculate_data_training(self):
        connection = pymysql.connect(host='localhost',
                                     user='root',
                                     password='Iamironman123',
                                     db='riyanlasso2')
        ...
        for cekTabel in tables:
            with connection.cursor() as cursor:
                for i in range(len(pilihMarker)):
                    sql = f"SELECT {cekTabel}.Frame,{cekTabel}.Name, {cekTabel}.Marker, {cekTabel}.X_Cordinates, {cekTabel}.X_Scoring, {cekTabel}.Y_Cordinates, {cekTabel}.Y_Scoring, {cekTabel}.Z_Cordinates, {cekTabel}.Z_Scoring FROM {cekTabel} where {cekTabel}.Marker = '{pilihMarker[i]}'"
                    cursor.execute(sql)
                    result = cursor.fetchall()
                    for row in result:
                        # print(f'Frame = {frame} Nama = {name} nilai marker = {marker} dan nilai pilihmarker = {pilihMarker[i]}')
                        frame = row[0]
                        frame_no_list.append(frame)
                        name = row[1]
                        marker = row[2]
                        
                        if marker == pilihMarker[i]:
                            x = row[3]
                            x_scoring = row[4]
                            ...
                            z = row[7]
                            z_scoring = row[8]
                            # print(frame, name, marker, x, x_scoring,y, y_scoring, z, z_scoring)
